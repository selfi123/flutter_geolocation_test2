  ...
  Future<void> _uploadLocationToFirestore() async {
    try {
      if (_currentLocation != null) {
        ...
        // Replace 'coordinates' with your Firestore collection name
        DocumentReference locationRef = firestore.doc('coordinates/location');

        await locationRef.set({
          'location': GeoPoint(_currentLocation!.latitude, _currentLocation!.longitude),
          'time': Timestamp.now(),
        }, SetOptions(merge: true)); // Use set with merge option to update the document
